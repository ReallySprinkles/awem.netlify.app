...
export default async (req) => {
  ...
  // Pass through any non-music request
  if (!pathname.includes("/music/") && !pathname.includes("/chart/music/")) {
    return; 
  }

  // Cover image schema with proper ByteDance structure
  const sampleCover = {
    uri: "finn_pfp",
    url_list: [
  ...
  // Items formatted with both flat and nested wrappers (`music` + `music_info`)
  ...
  // Hot Song category populate, My Favorites empty
  const mcList = [
    {
      mc_info: {
        mc_id: "1001",
